...
local function load_config_schema(plugin_t)
  ...
  if plugin_name then
    ...
    if constants.DEPRECATED_PLUGINS[plugin_name] then
      ngx.log(ngx.WARN, "the '" .. plugin_name .. "' plugin has been deprecated")
    end
    ...
    if singletons.configuration and not singletons.configuration.plugins[plugin_name] then
      return nil, 'Plugin "' .. tostring(plugin_name) ..
      '" not enabled, consider adding it with custom_plugins configuration'
    end
